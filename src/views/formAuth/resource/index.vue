...
export default {
  ...
  methods: {
    ...
    deleteMenu(row) {
      ...
      this.$confirm(`确认删除${row.action}权限吗？`, "删除权限", {
        cancelButtonText: "放弃",
        confirmButtonText: "删除"
      })
        .then(() => {
          deleteResourceManager(data, res => {
    ...
    searchMenu() {
      ...
      this.tempData = this.allData.filter(item => {
        let domain = item.domain ? item.domain.toLowerCase().trim() : "";
        let region = item.region ? item.region.toLowerCase().trim() : "";
        let action = item.action.toLowerCase().trim();
        if (
          domain.indexOf(s) != -1 ||
          action.indexOf(s) != -1 ||
          region.indexOf(s) != -1
